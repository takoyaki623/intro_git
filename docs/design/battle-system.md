...
「v0.3〜v0.5」と幅を持たせていたが、**v0.3 のバトルタワーが `BattleSet`（持ち物を含む）を
要求する**ため、幅のままでは成立しない。以下に確定する。

| 要素 | 導入 | 根拠 |
| --- | --- | --- |
| **持ち物** | **v0.3（必須）** | `BattleSet` が持ち物前提（[`endgame.md`](endgame.md) §6） |
| **特性** | **v0.3（必須）** | 同上。`BattleSet.ability` がある |

> **v0.3 で入れるのは「機構」であって、全種類ではない。**
> 特性は約310種、持ち物は数百あるが、v0.3 で必要なのは
...
持ち物前提）なので、v0.3 で必ず入れる。**そのためのフックだけは v0.1 で用意しておく。**
