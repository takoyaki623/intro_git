...
export type MoveEffect =
   ...
   */
  /**
   * プレゼント（v1.1-k）。**威力が実行時に決まる技の1件目。**
   *
   * 40%で威力40・30%で80・10%で120、残り20%は**相手を回復させる**。
   * 抽選は `resolvePresent` が行い、ここは登録のためだけに在る（`multiHit` と同じ形）。
   *
   * デリバードを野生に出すために要る ―― あの種は**レベルで覚える技がこれ1本**しかなく、
   * 実装しないと「技を1つも使えない野生」になって検証が落ちる（はねる と同じ理由）。
   */
  | { kind: "present" }
  | { kind: "transform" }
  /**
