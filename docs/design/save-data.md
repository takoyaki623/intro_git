...
### 9.10 v1.1-b ― スキーマは v5 のまま

わざマシン・進化の石・つながりのヒモは、**保存の形を1つも変えていない。**

| 変わったもの | どこに入るか |
| --- | --- |
| 覚えた技 | `PokemonInstance.moves`（v0.8 からある） |
| 進化した種族 | `PokemonInstance.species`（同上） |
| 持っているマシン・石 | `global.bag`（v0.9 からある） |
| じてんしゃ | 同じく `global.bag`。**速さは保存しない**（持っているかどうかだけ） |

**「新しい機構＝新しいスキーマ」ではない。** 道具が触るのは既にある器の中身で、
器そのものは増えていない ―― v6 が要るのは、この判断を覆したときだけ
（[`game-plan.md`](../game-plan.md) §8.1 の v1.1 の行に「v5 のまま」と書いてある理由）。
