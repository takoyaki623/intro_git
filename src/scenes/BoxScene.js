...
export class BoxScene {
  ...
  render(ctx) {
    ...
    drawText(ctx, `ボックス ${this.boxIndex + 1}`, 12, 9, { color: COL.ink });
    const used = this.currentBox.filter(Boolean).length;
    drawTextRight(ctx, `${used}/${BOX_SIZE}`, 244, 9, { color: COL.ink });

    // グリッド
    drawWindow(ctx, 4, 30, 248, 120);
    for (let i = 0; i < BOX_SIZE; i++) {
      const cx = GX + (i % COLS) * (CELL + 17);
      const cy = GY + Math.floor(i / COLS) * CELL;
      const mon = this.currentBox[i];
      const sel = this.area === 'box' && this.cursor === i;

      ctx.fillStyle = sel ? 'rgba(224,64,48,0.30)' : 'rgba(0,0,0,0.06)';
      ctx.fillRect(cx - 2, cy - 1, CELL, CELL - 2);

      if (mon) drawSprite(ctx, mon.species.sprite, cx - 2, cy - 3, { scale: 0.85 });
    }
    ...
    for (let i = 0; i < PARTY_MAX; i++) {
      ...
      const sel = this.area === 'party' && this.cursor === i;

      ctx.fillStyle = sel ? 'rgba(224,64,48,0.30)' : 'rgba(0,0,0,0.06)';
      ctx.fillRect(cx - 4, 156, 36, 28);
      if (!mon) continue;
      drawSprite(ctx, mon.species.sprite, cx - 2, 155, { scale: 1 });
      drawBar(ctx, cx - 2, 180, 24, mon.curHP / mon.stats.hp, hpColor(mon.curHP, mon.stats.hp), { h: 2, frame: false });
    }

    // 持っている個体はカーソルの上に浮かせる
    if (this.held) {
      const p = this.cursorPixel();
      drawSprite(ctx, this.held.mon.species.sprite, p.x - 4, p.y - 12, { scale: 1 });
    }

    // 説明
    const mon = this.held?.mon ?? this.slotMon();
    if (this.box) { this.box.render(ctx); return; }

    if (mon) {
      drawText(ctx, `${displayName(mon)}  Lv${mon.level}`, 12, 16, { color: COL.ink });
    }
    drawTextRight(ctx, this.held ? 'A ： おく  B ： もどす' : 'A ： つかむ  B ： とじる', 244, 16, {
      color: COL.inkLight,
    });
  }
  ...
  cursorPixel() {
    if (this.area === 'box') {
      return {
        x: GX + (this.cursor % COLS) * (CELL + 17),
        y: GY + Math.floor(this.cursor / COLS) * CELL,
      };
    }
    return { x: 12 + this.cursor * 40, y: 158 };
  }
}
